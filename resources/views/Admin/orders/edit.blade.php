        ...
        <div class="card mb-5 mb-xl-10">
            ...
            <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
                ...
                <div class="card-title m-0">
                    <h3 class="fw-bolder m-0">{{__('lang.Orders_Edit')}}</h3>
                </div>
                <!--end::Card title-->
            </div>
            <!--begin::Card header-->
            ...
            <div id="kt_account_settings_profile_details" class="collapse show">
                <!--begin::Form-->
                <form id="kt_account_profile_details_form" action="{{url($route.'/update/'.$data->id)}}" class="form"
                    method="post">
                    @csrf
                    <!--begin::Card body-->
                    <div class="card-body border-top p-9">
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <select name="car_id" class="form-control form-control-lg form-control-solid" required>
                                @foreach ($cars as $car)
                                <option value="{{$car->id}}" @if(isset($data->
                                    car_id))@if($data->car_id==$car->id)selected @endif
                                    @endif>@if(Session::get('lang')=='ar'){{ $car->name_ar }}@else{{ $car->name_en
                                    }}@endif</option>
                                @endforeach
                            </select>
                            <!--end::Input-->
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <input type="text" name="first_amenities"
                                ...
                                value="{{old('first_amenities',$data->first_amenities ?? '')}}" />
                            <!--end::Input-->
                        </div>
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <input type="text" name="second_amenities"
                                class="form-control form-control-solid mb-3 mb-lg-0" placeholder=""
                                value="{{old('second_amenities',$data->second_amenities ?? '')}}" />
                            <!--end::Input-->
                        </div>
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <input type="text" name="third_amenities"
                                class="form-control form-control-solid mb-3 mb-lg-0" placeholder=""
                                value="{{old('third_amenities',$data->third_amenities ?? '')}}" />
                            <!--end::Input-->
                        </div>
                        <div class="fv-row mb-7">
                            <!--begin::Label-->
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <select name="city_id" class="form-control form-control-lg form-control-solid" required>
                                ...
                                <option value="{{$city->id}}" @if(isset($data->
                                    city_id))@if($data->city_id==$city->id)selected @endif
                                    @endif>@if(Session::get('lang')=='ar'){{ $city->name_ar }}@else{{ $city->name_en
                                    }}@endif</option>
                                @endforeach
                            </select>
                            <!--end::Input-->
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <select name="type" class="form-control form-control-lg form-control-solid">
                                <option value="export" @if($data->type=="export")selected @endif>export</option>
                                <option value="import" @if($data->type=="import")selected @endif>import</option>
                            </select>
                            <!--end::Input-->
                        </div>
                        @foreach (App\Models\OrderDetail::where('order_id',$data->id)->get() as $key=>$t)

                        <div class="row" id="inputFormRow2{{ $key }}">
                            <div class="col-md-4">
                                <div class="fv-row mb-7">
                                    <label class="form-label">categories</label>
                                    <select name="category_id[]" class="form-control form-control-lg form-control-solid"
                                        id="category_id2{{ $key }}" required onclick="myFunctionsubs(this,{{$key}})">
                                        @foreach ($categories as $category)
                                        <option value="{{$category->id}}" @if($t->category_id==$category->id) selected
                                            @endif>@if(Session::get('lang')=='ar'){{ $category->name_ar }}@else{{
                                            $category->name_en }}@endif</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="fv-row mb-7">
                                    ...
                                    <select name="subcategory_id[]"
                                        ...
                                        id="subcategory_id2{{ $key }}" required>
                                        @foreach ($subcategories as $subcategory)
                                        <option value="{{$subcategory->id}}" @if($t->subcategory_id==$subcategory->id)
                                            selected @endif>@if(Session::get('lang')=='ar'){{ $subcategory->name_ar
                                            }}@else{{ $subcategory->name_en }}@endif</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="fv-row mb-7">
                                    ...
                                    <input type="number" name="number[]"
                                        class="form-control form-control-solid mb-3 mb-lg-0" placeholder=""
                                        value="{{old('number',$t->number ?? '')}}" required />
                                    <!--end::Input-->
                                </div>
                            </div>
                            <div class="input-group-append">
                                <button id="removeRow{{ $key }}" type="button"
                                value="{{ $key }}"
                                    class="btn btn-danger" onclick="removeDiv({{ $key }})">Remove</button>
                            </div>
                                                        {{--  <input type="hidden" id="countValue" value="{{ $key }}" />  --}}
                        </div>
                        ...
                        <br> --}}
                        <div id="newRow">
                            <input type="hidden" id="countValue" value="{{ $key+1 }}" />
                        </div>

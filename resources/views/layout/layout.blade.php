...
<script>
    function myFunction(id, e) {
        var elementCounter = $('#' + id.id).attr('element-counter');
        //console.log(elementCounter);
        var category_id = id.value;
        //alert(company_id);
        if (category_id) {
            {{--  console.log(category_id,e);  --}}
            $.ajax({
                type: "GET",
                // url:"{{url('get-category-units/')}}/?category_id="+category_id,
                url: "/subcategories/" + category_id,
                success: function (res) {
                    {{--  console.log(res,'res succu');  --}}

                    if (res) {
                        {{--  console.log(res);  --}}
                        $("#subcategory_id" + elementCounter).empty();
                        res.forEach(data => {
                            // console.log(elementCounter);
                            $("#subcategory_id" + elementCounter).append('<option value="' + data.id + '" >' + data.name + '</option>');
                        })
            ...
            $("#category_id" + elementCounter).empty();
        }
    }
</script>
        ...
        if (category_id) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res) {
                        {{--  console.log(res);  --}}
                        $("#subcategory_id2" + e).empty();
                        res.forEach(data => {
                            // console.log(elementCounter);
                            $("#subcategory_id2" + e).append('<option value="' + data.id + '" >' + data.name + '</option>');
                        })
...
<script>
    function myFunction3(e) {
        // var elementCounter = $('#'+e.e).attr('element-counter');
        //console.log(elementCounter);
...
<script type="text/javascript">
    ...
    $("#addRow").click(function () {
        ...
        var countValue = parseInt($('#countValue').val()) + 1;
        var category_id = parseInt($('#category_id').val());


        $.ajax({
            url: "/add-new-row/" + countValue + "/" + category_id,
            dataType: 'html',
            type: 'get',
            success: function (data) {
                $('#newRow').append(data);
                document.getElementById('countValue').value = countValue;
        ...
        {{--html += '<div class="row">';--}}
        {{--html += '<div class="col-md-4"><label class="form-label">categories</label><select name="category_id[]" class="form-control form-control-lg form-control-solid" id="category_id' + countValue + '" element-counter="'+ countValue +'" required onclick="myFunction(this, countValue.value)">@foreach (\App\Models\Category::whereNull('parent_id')->get() as $category)<option value="{{$category->id}}">@if(Session::get('lang')=='ar'){{ $category->name_ar }}@else{{ $category->name_en }}@endif</option>@endforeach</select></div>';--}}
        {{--html += '<div class="col-md-4"><label class="form-label">sub categories</label><select name="subcategory_id[]" class="form-control form-control-lg form-control-solid" id="subcategory_id' +countValue + '" required>@foreach (\App\Models\Category::whereNotNull('parent_id')->get() as $subcategory)<option value="{{$subcategory->id}}">@if(Session::get('lang')=='ar'){{ $subcategory->name_ar }}@else{{ $subcategory->name_en }}@endif</option>@endforeach</select></div>';--}}
        {{--html += '<div class="col-md-2"><label class="required fw-bold fs-6 mb-2">{{__('lang.number')}}</label><input type="number" name="number[]" class="form-control form-control-solid mb-3 mb-lg-0"placeholder="" value="{{old('number',$data->number ?? '')}}" required/></div>';--}}
        {{--html += '<div class="col-md-2"><div class="input-group-append"><label class=" fw-bold fs-6 mb-2">options</label><button id="removeRow" type="button" class="btn btn-danger">Remove</button></div></div>';--}}
        {{--html += '</div';--}}
        {{--html += '</div>';--}}
        {{--html += '';--}}
